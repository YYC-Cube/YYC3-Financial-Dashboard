/**
 * @file AI Family 数据常量
 * @description 8 位家人人格、系统命令、提示词预设、技能/插件/模块
 * @author YYC³
 * @version 1.0.0
 * @created 2026-07-16
...
import {
  ...
  Heart, Music2, Key, Sliders,
} from "lucide-react"
import type { FamilyPersona, SystemCommand, PromptPreset, ModelOption, AISkill, AIPlugin, AIModule, CommandCategory } from "./data-types"
...
export const SYSTEM_COMMANDS: (SystemCommand & { persona?: string })[] = [
  { id: "cmd-01", icon: Activity, label: "集群状态总览",   desc: "获取所有节点实时状态",               category: "cluster", action: "查看当前集群所有节点的运行状态、GPU利用率和温度", color: "#00d4ff", persona: "meta-oracle" },
  { id: "cmd-02", icon: Server,   label: "重启异常节点",   desc: "自动检测并重启异常节点",              category: "cluster", action: "检测并重启所有状态异常的推理节点", color: "#ff6600", persona: "meta-oracle" },
  { id: "cmd-03", icon: Layers,   label: "部署模型",       desc: "将模型部署到指定节点",                category: "model",   action: "将 DeepSeek-V3 模型部署到空闲 GPU 节点", color: "#00ff88", persona: "master" },
  { id: "cmd-04", icon: Cpu,      label: "推理性能报告",   desc: "生成推理性能分析报告",                category: "model",   action: "生成过去24小时的推理性能分析报告", color: "#aa55ff", persona: "thinker" },
  { id: "cmd-05", icon: Database, label: "数据库健康检查", desc: "检查 PostgreSQL 连接状态",            category: "data",    action: "执行数据库健康检查，检测连接池和慢查询", color: "#ffdd00", persona: "thinker" },
  { id: "cmd-06", icon: HardDrive, label: "存储空间分析",  desc: "分析存储使用和清理建议",               category: "data",    action: "分析当前存储空间使用情况给出清理建议", color: "#ff3366", persona: "thinker" },
  { id: "cmd-07", icon: Shield,   label: "安全审计扫描",   desc: "扫描安全漏洞和异常访问",              category: "security", action: "执行安全审计扫描，检查异常访问和潜在风险", color: "#ff3366", persona: "sentinel" },
  { id: "cmd-08", icon: Network,  label: "网络延迟诊断",   desc: "诊断节点间网络延迟",                 category: "monitor",  action: "诊断所有节点间的网络延迟和带宽状态", color: "#00d4ff", persona: "sentinel" },
  { id: "cmd-09", icon: Zap,      label: "一键优化配置",   desc: "AI 自动优化系统配置",                 category: "cluster", action: "根据当前负载情况，AI 自动优化集群配置参数", color: "#00ff88", persona: "meta-oracle" },
  { id: "cmd-10", icon: RotateCcw, label: "WebSocket 重连", desc: "重新建立数据推送连接",               category: "monitor",  action: "重新建立 WebSocket 实时数据推送连接", color: "#aa55ff", persona: "navigator" },
  { id: "cmd-11", icon: MessageSquare, label: "翻译助手",   desc: "多语言翻译与转写",                    category: "data",    action: "翻译以下内容为英文...", color: "#FFD700", persona: "navigator" },
  { id: "cmd-12", icon: Palette,  label: "创意提案",       desc: "生成创意方案与设计建议",              category: "model",   action: "针对当前场景生成创意方案", color: "#FF7043", persona: "creative" },
]
...
export const AI_SKILLS: AISkill[] = [
  ...
  { id: "skill-08", name: "知识检索",   description: "文档问答、知识库检索、信息聚合",              icon: BookOpen,  category: "nlp",     color: "#C0C0C0", active: true },
]
