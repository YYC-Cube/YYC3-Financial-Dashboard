/**
 * @file: FamilyHomePage.tsx
 * @description: AI Family 家园首页 — 时钟环布局 + 家人状态墙
 *
 * 从 src/app/components/ai-family/FamilyHome.tsx 迁移
 * 精简版: 展示 8 位家人卡片，点击切换人格
 */
import React, { useState } from "react";
import { Bot, Sparkles } from "lucide-react";
import { FAMILY_PERSONAS, PERSONAS_MAP } from "../data";
...
export default function FamilyHomePage() {
  ...
  // 时钟环计算
  const centerX = 180, centerY = 180, radius = 130;
  const angleStep = (2 * Math.PI) / FAMILY_PERSONAS.length;

  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-8"
      style={{ background: "radial-gradient(ellipse at center, rgba(0,255,136,0.03) 0%, rgba(4,8,20,1) 70%)" }}>
      ...
      <div className="relative" style={{ width: 360, height: 360 }}>
        ...
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 360 360">
          {FAMILY_PERSONAS.map((p, i) => {
            ...
            return (
              <line key={p.id} x1={centerX} y1={centerY} x2={x} y2={y}
        ...
        {FAMILY_PERSONAS.map((p, i) => {
          ...
          return (
            <button key={p.id} onClick={() => handlePersonaClick(p.id)}
              ...
              <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 whitespace-nowrap">
                <span className="text-[rgba(0,255,136,0.4)]" style={{ fontSize: "0.6rem" }}>{p.shortName}</span>
              </div>
            </button>
          );
        })}
      ...
      {/* 底部状态 */}
      <div className="mt-20 flex items-center gap-4 text-[rgba(0,255,136,0.2)]" style={{ fontSize: "0.6rem" }}>
        <span>点击家人打开中枢对话</span>
        <span>·</span>
        <span>{FAMILY_PERSONAS.filter(p => p.mood !== "idle").length} 人在线</span>
      </div>
    </div>
  );
}
